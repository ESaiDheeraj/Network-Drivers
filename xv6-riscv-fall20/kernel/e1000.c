...
e1000_transmit(struct mbuf *m)
{
  ...
  if(next == head) {
    // to ensure ring doesn't get overflow
    panic("full tx_ring\n");
  }
...
e1000_recv(void)
{
  ...
  while((rx_ring[head].status & E1000_RXD_STAT_DD)) 
  {
    ...
    // updating the mbuf's length to the length reported in the descriptor
    mbufput(mbuf, rx_ring[head].length);

    // allocate a new mbuf and program its head pointer into the descripter and clean the status
  ...
  // put an assert to confirm head and tail coincide ???
